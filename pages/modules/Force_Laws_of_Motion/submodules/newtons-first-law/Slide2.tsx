...
const StateOfMotionAnimation = ({ isDarkMode }: { isDarkMode: boolean }) => {
    ...
    // --- NEW: SVG Icons for thruster buttons ---
        ...
        <div className={`w-full h-full ${isDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 mx-auto max-w-7xl h-full">
                <div className="space-y-6 flex flex-col justify-center">
                    <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-xl p-6 shadow-lg`}><h3 className="text-2xl font-bold mb-4 text-blue-500">The Two States of Equilibrium</h3><p className="text-lg leading-relaxed">Newton's First Law describes two states where an object's motion is stable and unchanging. In both states, the <strong>net force</strong> on the object is zero.</p></div>
                    <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-xl p-6 shadow-lg`}><h3 className="text-xl font-semibold mb-4 text-blue-600 dark:text-blue-400">1. State of Rest</h3><p className="text-lg leading-relaxed">An object at rest has a velocity of zero. It will remain at rest forever unless an unbalanced force acts on it.</p></div>
                    <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-xl p-6 shadow-lg`}>
                        <h3 className="text-xl font-semibold mb-4 text-blue-600 dark:text-blue-400">2. Uniform Motion</h3>
                        <p className="text-lg leading-relaxed">An object in uniform motion moves with a <strong>constant velocity</strong> (constant speed and constant direction). It will continue this motion forever unless an unbalanced force acts on it.</p>
                        <div className="mt-4 p-4 bg-slate-100 dark:bg-slate-700 rounded-lg"><p className="font-semibold text-center">A force is NOT needed to keep an object moving. A force is only needed to CHANGE its motion (to accelerate it).</p></div>
                    </div>
                </div>
