...
export default function WordProblemsSlide() {
    ...
    const [selection, setSelection] = useState<'add' | 'subtract' | null>(null);
    ...
    const handleSelection = (op: 'add' | 'subtract') => {
        setSelection(op);
        setIsCorrect(op === problem.op);
    };

    const slideInteractions: Interaction[] = [
        { id: 'word-problems-20', conceptId: 'word-problems-20', conceptName: 'Word Problems within 20', type: 'judging' }
    ];
             ...
             <div className="bg-blue-50 dark:bg-blue-900/30 border-l-4 border-blue-400 dark:border-blue-700 rounded-lg px-6 py-4">
                ...
                <p className="text-slate-600 dark:text-slate-400">Word problems are math stories! Look for clue words to decide if you need to add or subtract.</p>
                 <ul className="list-disc pl-5 mt-2 space-y-1">
                    <li>Words like "in all," "altogether," or "more" usually mean you need to **add (+)**.</li>
                    <li>Words like "how many are left" or "how many more" usually mean you need to **subtract (-)**.</li>
                </ul>
            </div>
        </div>
    );
            ...
            <p className="text-slate-600 dark:text-slate-400 mb-4">Read the story and choose the right operation!</p>
             <div className="bg-slate-100 dark:bg-slate-900/70 rounded-lg p-6 flex-grow flex flex-col justify-center items-center text-center">
                ...
                <div className="flex gap-4">
                    <button onClick={() => handleSelection('add')} disabled={selection !== null} className={`w-24 h-24 text-6xl rounded-lg font-bold transition-colors ${selection === 'add' ? (isCorrect ? 'bg-blue-500 text-white' : 'bg-red-500 text-white') : 'bg-slate-200 dark:bg-slate-700 hover:bg-slate-300'}`}>+</button>
                    <button onClick={() => handleSelection('subtract')} disabled={selection !== null} className={`w-24 h-24 text-6xl rounded-lg font-bold transition-colors ${selection === 'subtract' ? (isCorrect ? 'bg-blue-500 text-white' : 'bg-red-500 text-white') : 'bg-slate-200 dark:bg-slate-700 hover:bg-slate-300'}`}>-</button>
                </div>
                <AnimatePresence>
                {isCorrect !== null && (
    ...
    return (
        <SlideComponentWrapper slideId="word-problems-within-20" slideTitle="Word Problems within 20" moduleId="grade-2-math-operations" submoduleId="add-subtract-within-20" interactions={localInteractions}>
            {slideContent}
