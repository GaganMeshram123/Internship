    ...
    <div className={`w-full min-h-screen ${isDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 mx-auto max-w-7xl">
        <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-xl p-6 shadow-lg`}>
            ...
            <h3 className="text-2xl font-bold mb-4 text-blue-500">Telling a Complex Story</h3>
            ...
            <div className="p-4 rounded-lg bg-slate-100 dark:bg-slate-700">
              <svg viewBox="0 0 100 60" className="w-full">
                ...
                {/* STYLE UPDATE: Graph line color changed to blue */}
                <path d="M 5 55 L 30 10 L 65 10 L 90 55" stroke="#3b82f6" strokeWidth="2" fill="none" />
                {/* Labels */}
                ...
                {/* STYLE UPDATE: Text colors on graph changed */}
                <text x="10" y="40" fill={isDarkMode ? '#93c5fd' : '#3b82f6'} fontSize="4">Speeding Up</text>
                <text x="35" y="5" fill={isDarkMode ? '#93c5fd' : '#3b82f6'} fontSize="4">Constant Velocity</text>
                <text x="68" y="40" fill={isDarkMode ? '#93c5fd' : '#3b82f6'} fontSize="4">Slowing Down</text>
              </svg>
            </div>
        ...
        <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-xl p-6 shadow-lg`}>
          ...
          <div className="flex justify-between items-center mb-4"><h3 className="text-xl font-semibold text-blue-600 dark:text-blue-400">Analyze the Story</h3><div className="text-lg text-slate-600 dark:text-slate-400">Question {isQuizComplete ? questions.length : currentQuestionIndex + 1} of {questions.length}</div></div>
          ...
          {!isQuizComplete ? ( <> <p className="text-lg mb-6 min-h-[6rem]">{questions[currentQuestionIndex].question}</p><div className="space-y-3">{questions[currentQuestionIndex].options.map((option) => (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className={`mt-4 p-4 rounded-lg bg-blue-100 dark:bg-blue-900/30 border border-blue-300 dark:border-blue-700`}>
              <div className="font-bold text-lg mb-2">{selectedAnswer === questions[currentQuestionIndex].correctAnswer ? 'Correct Interpretation!' : 'Look closer at the graph.'}</div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8">
            ...
            <div className="text-2xl font-bold mb-2 text-blue-600 dark:text-blue-400">Story Read!</div>
