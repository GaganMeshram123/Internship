      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 mx-auto max-w-7xl">
        <div className="space-y-6">
          <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-xl p-6 shadow-lg`}>
            ...
            <h3 className="text-2xl font-bold mb-4 text-blue-500">From Knowledge to Action</h3>
          ...
          <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-xl p-6 shadow-lg`}>
            ...
            <h3 className="text-xl font-semibold mb-4 text-blue-600 dark:text-blue-400">The G.U.E.S.S. Method</h3>
            <p className="text-lg leading-relaxed mb-4">
              Follow these five steps every time, and you'll become an expert problem-solver.
            </p>
            <ul className="space-y-3 text-lg">
                {/* STYLE UPDATE: List item highlight color changed to blue */}
                <li className="p-3 bg-slate-100 dark:bg-slate-700 rounded-lg"><strong className="text-blue-600 dark:text-blue-400">G</strong> - <strong>Givens:</strong> List every piece of information and its variable (<InlineMath math="u, v, a, s, t"/>).</li>
                <li className="p-3 bg-slate-100 dark:bg-slate-700 rounded-lg"><strong className="text-blue-600 dark:text-blue-400">U</strong> - <strong>Unknown:</strong> Identify the one variable you need to find.</li>
                <li className="p-3 bg-slate-100 dark:bg-slate-700 rounded-lg"><strong className="text-blue-600 dark:text-blue-400">E</strong> - <strong>Equation:</strong> Choose the kinematic equation that connects your givens and unknown.</li>
                <li className="p-3 bg-slate-100 dark:bg-slate-700 rounded-lg"><strong className="text-blue-600 dark:text-blue-400">S</strong> - <strong>Substitute:</strong> Plug your given values into the equation.</li>
                <li className="p-3 bg-slate-100 dark:bg-slate-700 rounded-lg"><strong className="text-blue-600 dark:text-blue-400">S</strong> - <strong>Solve:</strong> Calculate the final answer and include units!</li>
            </ul>
          </div>
        </div>
        ...
        <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-xl p-6 shadow-lg`}>
          {/* STYLE UPDATE: Subheading style applied */}
          <div className="flex justify-between items-center mb-4"><h3 className="text-xl font-semibold text-blue-600 dark:text-blue-400">Master the Strategy</h3><div className="text-lg text-slate-600 dark:text-slate-400">Question {isQuizComplete ? questions.length : currentQuestionIndex + 1} of {questions.length}</div></div>
          ...
          {!isQuizComplete ? ( <> <p className="text-lg mb-6 min-h-[6rem]">{questions[currentQuestionIndex].question}</p><div className="space-y-3">{questions[currentQuestionIndex].options.map((option) => (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className={`mt-4 p-4 rounded-lg bg-blue-100 dark:bg-blue-900/30 border border-blue-300 dark:border-blue-700`}>
              <div className="font-bold text-lg mb-2">{selectedAnswer === questions[currentQuestionIndex].correctAnswer ? 'That\'s the key!' : 'Think about the first step.'}</div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8">
            ...
            <div className="text-2xl font-bold mb-2 text-blue-600 dark:text-blue-400">Strategy Learned!</div>
