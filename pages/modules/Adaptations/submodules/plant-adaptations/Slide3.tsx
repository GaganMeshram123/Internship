...
export default function PlantAdaptationsSlide3() {
  ...
  const questions: QuizQuestion[] = [
    {
      id: 'phototropism-question',
      question: 'A plant\'s stems and leaves growing toward a light source is an example of what?',
      options: [
  ...
  const handleQuizAnswer = (answerText: string) => {
    ...
    handleInteractionComplete({
      interactionId: `plant-tropism-quiz-q${currentQuestionIndex + 1}-${current.id}-${Date.now()}`,
      value: answerText,
      isCorrect,
      timestamp: Date.now(),
      conceptId: 'plant-adaptation-understanding',
            ...
            {!isQuizComplete ? (
              ...
              <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8">
                ...
                <div className="text-lg text-slate-600 dark:text-slate-400 mt-2">
                  {score === questions.length ? 'Perfect! You have a green thumb! 🌱' : 'Great job! 👏'}
                </div>
              </motion.div>
            )}
          </div>
  ...
  return (
    <SlideComponentWrapper 
      slideId="behavioral-tropism"
      slideTitle="Behavioral: Tropism"
